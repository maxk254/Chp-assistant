...
const LiveMap = ({ patients = [] }) => {
  ...
  const supervisors = [
    ...
    {
      id: "s3",
      name: "Dr. Moses Kipchoge",
      region: "Rift Valley",
      x: 380,
      y: 420,
      contact: "+254 722 654 321",
    },
  ];
  ...
  const referrals = patients
    .filter((p) => !selectedWard || p.ward === selectedWard)
    .map((p, i) => ({
      id: `ref-${i}`,
      patient: p.fullName,
      ward: p.ward,
  ...
  const filteredFacilities = selectedWard
    ? facilities.filter(
        (f) =>
          Object.values(wardToFacility).includes(f.id) &&
          wardToFacility[selectedWard] === f.id,
      )
    : facilities;
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-400 mt-1 font-medium">
            ...
            | Hospitals:{" "}
            <span className="text-blue-400 font-bold">{facilities.length}</span>
          </p>
        </div>
        <button
          onClick={runAnalysis}
          ...
          title={
            selectedWard ? "Click to optimize referrals" : "Select a ward first"
          }
        >
          <Activity size={16} />
          <span>
            {selectedWard ? "Optimize Referrals" : "Select Ward First"}
          </span>
        </button>
      </div>
      ...
      <div className="flex-1 flex gap-8 min-h-0">
        ...
        <div className="flex-[3] bg-slate-900/40 border border-slate-700/50 rounded-[2.5rem] overflow-hidden relative shadow-2xl">
          ...
          <div className="absolute inset-0 bg-slate-950/60 p-10">
            <svg
              viewBox="0 0 800 600"
              className="w-full h-full opacity-40 fill-none stroke-slate-600 stroke-[2px]"
            >
              {/* Kenya outline (simplified) */}
              <path
                d="M100,80 L200,50 L350,40 L500,60 L650,80 L700,150 L720,300 L700,450 L600,520 L400,550 L200,540 L80,450 L50,250 Z"
                strokeWidth="2.5"
              />
            ...
            {filteredFacilities.map((f) => (
              ...
              >
                <div className="relative">
                  <div
                    className={`w-14 h-14 rounded-2xl bg-blue-500/20 flex items-center justify-center border border-blue-400/60 group-hover:scale-125 group-hover:bg-blue-500/40 transition-all shadow-lg ${f.color}`}
                  >
                    <Hospital size={28} />
                  </div>
            ...
            {chws.map((c) => (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center border-2 text-white hover:scale-125 transition-transform shadow-lg ${
                    c.status === "Active"
          ...
          {isAnalyzing && (
            <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-sm flex flex-col items-center justify-center z-50">
              ...
              <p className="text-slate-500 mt-2 text-xs">
                AI is calculating optimal referral trajectories
              </p>
            </div>
          )}
              ...
              {selectedWard && (
                <div className="mb-4 p-3 bg-teal-400/10 border border-teal-400/20 rounded-lg">
                  ...
                  <p className="text-white text-sm">
                    {KENYAN_WARDS.find((w) => w.value === selectedWard)
                      ?.label || selectedWard}
                  </p>
                </div>
              )}
                ...
                {analysisResult.ward && (
                  <div className="p-3 bg-slate-900/50 rounded-lg border border-teal-400/15 mb-2">
                    ...
                    <p className="text-teal-300 text-sm font-medium">
                      {KENYAN_WARDS.find((w) => w.value === analysisResult.ward)
                        ?.label || analysisResult.ward}
                    </p>
                  </div>
                )}
                <div className="p-4 bg-slate-900/50 rounded-2xl border border-teal-400/10">
                  <p className="text-teal-400 font-black text-lg">
                    {analysisResult.bestMatch.name}
                  </p>
                  <p className="text-slate-400 text-xs mt-1">
                    {analysisResult.bestMatch.type}
                  </p>
                  <div className="flex justify-between mt-3">
                    ...
                    <span className="text-slate-500 text-xs">
                      ETA:{" "}
                      <b className="text-emerald-500">{analysisResult.eta}</b>
                    </span>
                  </div>
                </div>
            ...
            <div className="bg-amber-400/5 border border-amber-400/20 rounded-3xl p-6 animate-in slide-in-from-right-4">
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto">
                {referrals.slice(0, 5).map((ref) => (
                  <div
                    key={ref.id}
                    className="p-3 bg-slate-900/50 rounded-lg border border-amber-400/10 text-sm"
                  >
            ...
            {selectedEntity ? (
              <div className="animate-in fade-in duration-500">
                <div className="flex items-center space-x-4 mb-6">
                  <div
                    className={`p-4 rounded-2xl bg-slate-900/50 border border-slate-700 ${selectedEntity.color || "text-emerald-500"}`}
                  >
